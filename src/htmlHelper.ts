...
export class HTMLHelper {
    ...
    public static createLayout(data: any, uiDetailsMap: Map<string, string>, isFormRequired: boolean) {
        ...
        if (isFormRequired) {
            layoutSnippet = "<div class=\"container\">\n\t<div class=\"row\">\n\t\t<h2>pageHeading</h2>" +
                "\n\t</div>\n\t<form [formGroup]=\"" + FileHelper.capitalizeWord(uiDetailsMap.get("componentName")!) + "Form\" #f=\"ngForm\" (ngSubmit)=\"onSubmit(f)\">";
        }
    ...
    public static generateCodeSnippet(data: any, index: number, htmlString: string, uiDetailsMap: Map<string, string>, hasStyleChecked: string) {
    ...
    public static getDivIdBySequence(seq: string, col: string, sec: string) {
        let num = Number(seq);
    ...
    public static getHtmlTextByType(data: any, htmlString: string) {
    ...
    public static getCustomControlsHTML(data: any, htmlString: string) {
        let retStr = "";
    ...
    public static addControls(htmlString: string, htmlControlsText: string[], divIds: string[], ctrlType: string, parentId: String) {
