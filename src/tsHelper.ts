...
export class TSHelper {
	...
	public static updateTypeScriptFile(data: any, htmlString: string, uiDetailsMap: Map<string, string>, tsStringArray: any[]) {
		if (data.style !== null && data.style !== undefined && data.style.trim() !== "") {
			return tsStringArray;
		}

		if (!tsStringArray[0]) {
			tsStringArray.splice(0, 1, FileHelper.getComponentString(uiDetailsMap));
			let commonAttr = "\n\t" + FileHelper.capitalizeWord(uiDetailsMap.get("componentName")!) + "Form: FormGroup;\n\tonSubmit(form) {\n\t\t\n\t}\n";
			...
			let consDet = "\n\tthis." + FileHelper.capitalizeWord(uiDetailsMap.get("componentName")!) + "Form = this.formBuilder.group({ \n\t });";
			tsStringArray.splice(0, 1, TSHelper.addFormGroupInConstructor(consDet, tsStringArray[0]));
		}
		if (!tsStringArray[1]) {
			tsStringArray.splice(1, 1, FileHelper.getServiceString(uiDetailsMap));
		}
		let retStringArray: any[] = ["", ""];
		if (data.controlType) {
	...
	static updateHTMLComponentChanges(data: any, tsStringArray: any[], uiDetailsMap: Map<string, string>) {
		let modStrArray: any[] = ["", ""];
		let compStr: string = "";
		let serStr: string = "";
		...
		switch (tp) {
			case "inputbox":
				...
				break;
			case "dropdown":
				let url: string = data.serviceCall;
				if (url) {
					url = url.replace(/\s/g, "");
				}
			...
			case "button":
				...
				break;
			case "submit":
				let urlStr: string = data.serviceCall;
				if (urlStr) {
					urlStr = urlStr.replace(/\s/g, "");
				}
				let cntS = "\n\ton" + tp + "Clicked($node,form){\n\t\tconsole.log('Submit Button Clicked.');" +
					"\n\t\tthis.formFieldsData = new FormEntry(this." + FileHelper.capitalizeWord(uiDetailsMap.get("componentName")!) + "Form.value);" +
			...
			case "checkbox":
				if (data.children) {
					let cntS = "\n\t\t@ViewChildren(\""+data.name+"Checkboxes\") "+data.name+"Checkboxes: QueryList<ElementRef>;"+
						...
						"\n\t\t\tconst checkedVals = (this." + FileHelper.capitalizeWord(uiDetailsMap.get("componentName")!) + "Form.controls." + data.controlId + " as FormArray);" +
	...
	static updateCustomComponentChanges(data: any, tsStringArray: any[], uiDetailsMap: Map<string, string>) {
		let modStrArray: any[] = ["", ""];
		let compStr: string = "";
		let serStr: string = "";
		var tp = data.controlType.replace(/\s/g, "").toLowerCase();
		let serContent: string = "";
		let tsContent: string = "";
		let serviceName = FileHelper.camelCase(FileHelper.capitalizeWord(uiDetailsMap.get("moduleName")!) + "Service");
		let urlArray: string = data.serviceCall;
		let url: string[] = [];
		if (urlArray) {
	...
	static modifyTSFileString(componentName: string, content: string, fileContent: string) {
		var txt = fileContent;
	...
	static updateFormBinding(componentName: string, content: string, fileContent: string) {
		var txt = fileContent;
	...
	static updateServiceTsFileString(compName: string, content: string, data: any[], moduleName: string, fileContent: string) {
	...
	static addInConstructorParamenter(content: string, fileContent: string, uiDetailsMap: Map<string, string>) {
	...
	static addFormGroupInConstructor(content: string, fileContent: string) {
		var txt = fileContent;
	...
	static updateImportInComponent(importMap: Map<string, string>, appModImpStr: string, compStr: string) {
		var txt = compStr;
		importMap.forEach((value: string, key: string) => {
	...
	static updateAppModuleString(content: string, content2: string, fileContent: string, onlyImport: boolean) {
		var txt = fileContent;
